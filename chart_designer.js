...
const actualContentColor = '#35006E';

let params;
let entityType;

export function initScopeChart(plannedItems, actualContentItems, Labels, trendDrillDownCacheId, paramsVal, entityTypeVal, widgetName='Scope Change Report', yAxisPrefix) {
    ...
    const yChartMakHeight = (Math.max(plannedItems, actualContentItems) + 3);
    ...
    const scopeChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Planned',
                    data: createDataObject(Labels, plannedItems, trendDrillDownCacheId, false),
                    backgroundColor: plannedColor
                },
                {
                    label: 'Actual Content',
                    data: createDataObject(Labels,actualContentItems, trendDrillDownCacheId, true),
                    backgroundColor: actualContentColor
                }
            ]
        },
        options: {
            ...
            onClick: async (evt, elements) => {
                if (elements.length > 0) {
                    ...
                    const category = [2,3];
                    const deletedItemsCategory = 2;

                    const entities = await getTrendDrillCacheWorkItems(params, entityType, value.drillDownCacheID, category, deletedItemsCategory, value.shouldFilterOnlyDone);
...
function createDataObject(labels, values, trendDrillDownCacheId, shouldFilterOnlyDone) {
    let returnVal = [];
    for (let i=0; i<values.length; i++) {
        returnVal.push({x: labels[i], y: values[i], drillDownCacheID: trendDrillDownCacheId[i], shouldFilterOnlyDone: shouldFilterOnlyDone});
    }
    return returnVal;
}
