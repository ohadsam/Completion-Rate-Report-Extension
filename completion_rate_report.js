...
async function createScopeChangeReport(releases, sprintId, teams) {
    ...
    let planned = [];
    let actualContent = [];
    let plannedTrendDrillDownCacheId = [];
        ...
        if (selectedXAxis === "xaxis-release") {
            ...
            for (const releaseID of releases) {
                ...
                if (release && release.start_date && release.end_date) {
                    ...
                    const reportData = await getCompletionRateData(selectedType, releaseStartDate, releaseEndDate, release.id, undefined, undefined, teams);
                    planned.push(reportData.planned);
                    actualContent.push(reportData.actualContent);
                    plannedTrendDrillDownCacheId.push(reportData.plannedCacheID);
                }
            }
            previousScopeChart = initScopeChart(planned, actualContent, Labels, plannedTrendDrillDownCacheId, params, selectedType, getGraphName('Release'), getYAxisLabelPrefix());
            showLoading(false);
            ...
            for (const releaseID of releases) {
                ...
                for (const sprint of Object.values(release['sprintMap'])) {
                    if (sprint && sprint.start_date && sprint.end_date) {
                        ...
                        const reportData = await getCompletionRateData(selectedType, sprintStartDate, sprintEndDate, release.id, sprint.id, undefined, teams);
                        planned.push(reportData.planned);
                        actualContent.push(reportData.actualContent);
                        plannedTrendDrillDownCacheId.push(reportData.plannedCacheID);
                    }
            ...
            previousScopeChart = initScopeChart(planned, actualContent, Labels, plannedTrendDrillDownCacheId, params, selectedType, getGraphName('Sprint'), getYAxisLabelPrefix());
            showLoading(false);
            ...
            for (const releaseID of releases) {
                ...
                for (const milestone of Object.values(release['milestoneMap'])) {
                    ...
                    if (milestone && milestone.date) {
                        const reportData = await getCompletionRateData(selectedType, milestoneStartDate, milestone.date, release.id, undefined, milestone.id, teams);
                        planned.push(reportData.planned);
                        actualContent.push(reportData.actualContent);
                        plannedTrendDrillDownCacheId.push(reportData.plannedCacheID);
                    }
            ...
            previousScopeChart = initScopeChart(planned, actualContent, Labels, plannedTrendDrillDownCacheId, params, selectedType, getGraphName('Milestone'), getYAxisLabelPrefix());
            showLoading(false);
        }
    } else{
        previousScopeChart = initScopeChart([0], [0], [],  [], params, selectedType);
        showLoading(false);
    }
}

async function getbackgroundTasktrendCacheID(dataFromPostTrendData) {
    if (dataFromPostTrendData.backgroundTask) {
        let bgSuccess = false;
        let i = 0;
        while (!bgSuccess && i < 30) {
            const bgEntity = await getBackgroundTask(params, dataFromPostTrendData.backgroundTask.id);
        ...
        if (bgSuccess && dataFromPostTrendData.trendDrillDownCacheId) {
            return dataFromPostTrendData.trendDrillDownCacheId;
        }
        return -1;
    }
}

async function getDataFromPostTrendData(trendDrillDownCacheId, entityType, filterOnlyDone) {
    let returnValue= 0;
    if (trendDrillDownCacheId > 0) {
        const removedEntities = await getTrendDrillCacheWorkItems(params, entityType, trendDrillDownCacheId, 3, 3, filterOnlyDone);
        const unChangedEntities = await getTrendDrillCacheWorkItems(params, entityType, trendDrillDownCacheId, 2, 2, filterOnlyDone);
        if (yaxisSelect.value === "yaxis-count") {
            returnValue = removedEntities.data.length + unChangedEntities.data.length;
            return {
                planned: returnValue,
                trendDrillDownCacheId: trendDrillDownCacheId
            }
        } else {
            ...
            let returnValue = collectEntitiesDataByField(unChangedEntities.data, sumSelectValue) + descoped;
            return {
                planned: returnValue,
                trendDrillDownCacheId: trendDrillDownCacheId
            }
        ...
        return {planned: returnValue};
    }
}

/*async function getDataFromPostTrendData(dataFromPostTrendData, entityType) {
    let returnValue= 0;
    if (dataFromPostTrendData.backgroundTask) {
        let bgSuccess = false;
        let i = 0;
        ...
        if (bgSuccess && dataFromPostTrendData.trendDrillDownCacheId) {
            ...
            if (yaxisSelect.value === "yaxis-count") {
                const groupedEntities = await getTrendDrillCacheGroups(params, dataFromPostTrendData.trendDrillDownCacheId);
                //we are interested only in descoped and unchanged items
                if (groupedEntities.groups && groupedEntities.groups.length > 0) {
                    groupedEntities.groups.forEach(group => {
                        if (group.value === 3 || group.value === 2) { //descoped items (3) and unchaneged items (2)
                            returnValue += group.count;
                        }
                    });
                }
                return {
                    planned: returnValue,
                    trendDrillDownCacheId: dataFromPostTrendData.trendDrillDownCacheId
                };
            } else { //if need to sum by story point fields
                const removedEntities = await getTrendDrillCacheWorkItems(params, entityType, dataFromPostTrendData.trendDrillDownCacheId, 3, 3);
                const unChangedEntities = await getTrendDrillCacheWorkItems(params, entityType, dataFromPostTrendData.trendDrillDownCacheId, 2, 2);
                const sumSelectValue = document.getElementById('sum-select').value;
...
async function getCompletionRateData(entityType, startDate, endDate, releaseId, sprintID, milestoneID, teams) {
    if (!startDate ||!endDate) {
        return {planned: 0, actualContent: 0};
    }
    const scopeData = await postTrendDataDrillDown(params, entityType, startDate, endDate, releaseId, sprintID, milestoneID, teams);
    const trendCacheID = await getbackgroundTasktrendCacheID(scopeData);
    const scopeDataValue = await getDataFromPostTrendData(trendCacheID, entityType, false);
    const actualContentDataValue = await getDataFromPostTrendData(trendCacheID, entityType, true);
    return {planned: scopeDataValue.planned, actualContent: actualContentDataValue.planned, plannedCacheID: scopeData.trendDrillDownCacheId};
}
